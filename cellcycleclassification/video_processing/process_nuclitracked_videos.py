...
def process_directory(working_dir: Path, export_frames: Path = False):
    """
    ...
    export_frames: Path, default True
    ...
    for csv_fname in tqdm(csv_fnames, desc='Processing video '):
        process_one(csv_fname)


def process_one(tracking_csv: Path, export_frames: Path = False):
    """
...
def process(input_path: Path, export_frames: Path = False):
    """
    ...
    if input_path.is_dir():
        process_directory(input_path, export_frames=export_frames)
    else:
        process_one(input_path, export_frames=export_frames)
